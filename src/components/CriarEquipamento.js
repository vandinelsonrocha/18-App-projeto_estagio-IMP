...
export default function CriarEquipamento({ visible, onClose, onCriar }) {
  ...
  const handleCriar = async () => {
    try {
      ...
      const doc = await db.collection('equipamentos').doc(codigoBarras).get();
      if (doc.exists) {
        Alert.alert("Erro", `Não foi possível criar porque um equipamento com o código de barras ${codigoBarras} já existe.`);
        return;
      }
      ...
      console.error("Erro ao criar o equipamento: ", erro);
